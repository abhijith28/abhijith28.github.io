...
$(function() {
    ...
    timeCaledarFlag = true
    jQuery(function(){

        jQuery('#event-start-date').datetimepicker({
            onShow:function( ct ){
                this.setOptions({
                    maxDate:jQuery('#event-end-date').val()?jQuery('#event-end-date').val():false
                })
                if (timeCalendarFlag)
                {

                    var startCalendarHour = (new Date).getHours()
                    var startCalendarMinute = (new Date).getMinutes()
                    var startCalendarSecond = (new Date).getSeconds()
                    startCalendarTime = new Date(2000,01,01,startCalendarHour,startCalendarMinute,startCalendarSecond)
                    $(document).trigger('log', ['Start Calendar Time', {startCalendarHour,startCalendarMinute,startCalendarSecond}]);

                    timeCalendarFlag = false
