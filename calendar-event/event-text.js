...
$(function() {
    ...
    $('#event-name').on('keydown', function(evt)
    {
        if (timeFlag)
        {
            ...
            startTime = new Date(2000,01,01,startHour,startMinute,startSecond)
            $(document).trigger('log', ['Start Event Time', {startHour,startMinute,startSecond}]);
	...
	$('#create-event-button').click(function() {
		...
		$(document).trigger('log', ['create event button click', {click: true}]);

		if (checkInputs()) {

			count += 1;
			timeFlag = true
			timeCalendarFlag = true
            var endHour = (new Date).getHours()
            var endMinute = (new Date).getMinutes()
            var endSecond = (new Date).getSeconds()
            var endTime = new Date(2000,01,01,endHour,endMinute,endSecond)
            $(document).trigger('log', ['End Event Time', {endHour,endMinute,endSecond}]);
			timeTaken = (endTime - startTime)/1000
            $(document).trigger('log', ['Time to create an event (seconds)', {timeTaken}]);

            $('#eventCount').html(count);
			...
			setTimeout(function(){
                ...
                // $("#create-event-button").css("background","#cccccc");
            ...
            $(document).trigger('log', ['success', {passed: true}]);

		}else {
            $(document).trigger('log', ['failure', {failed: true}]);
        }
	});
});
...
function checkInputs() {
	...
	if (!document.getElementById('all-day-event-checkbox').checked) {
        if (!isValidEndTime()) {
